...
# ─── Phase 4: Grounded Document Q&A / RAG Endpoint ────────────────────────────


@router.post("/{document_id}/chat", response_model=QAResponse)
async def chat_document_endpoint(document_id: str, request: QARequest):
    """
    Grounded Document Q&A endpoint.
    Retrieves context for the user question and returns a structured AI answer with page citations.
    """
    doc = get_document_by_id(document_id)
    if not doc:
        raise HTTPException(
    ...
    if doc.processing_status != ProcessingStatus.COMPLETED:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Document text extraction has not completed. Cannot run Q&A."
        )

    try:
        response = await rag_service.answer_question(document_id, request.question)
        return response
        ...
        logger.error(f"Error in document Q&A for {document_id}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
