...
interface ChatTabProps {
  ...
  analysisData?: import('@/app/workspace/[id]/page').LegalAnalysisData | null;
}
...
export function ChatTab({ documentId, onSelectCitation, analysisData }: ChatTabProps) {
  ...
  return (
    <div className="flex flex-col h-[640px] bg-slate-900/80 border border-slate-800 rounded-2xl overflow-hidden shadow-xl" role="region" aria-label="Ask Your Document chat interface">
      {/* Header Banner */}
      ...
      >
        <input
          ...
          autoComplete="off"
          className="flex-1 bg-slate-900 border border-slate-800 rounded-xl px-4 py-2.5 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-brand-500 focus-visible:ring-2 focus-visible:ring-brand-500 disabled:opacity-50"
        />
        <button
          type="submit"
          disabled={!input.trim() || loading}
          aria-label={loading ? 'Generating answer...' : 'Send question'}
          className="bg-brand-600 hover:bg-brand-500 disabled:opacity-40 text-white p-2.5 rounded-xl transition-colors shadow-glow flex items-center justify-center shrink-0 focus-visible:ring-2 focus-visible:ring-brand-400 focus-visible:outline-none"
        >
